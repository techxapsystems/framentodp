...
export async function getValidPasswordResetToken(token: string) {
  ...
  try {
    const { passwordResetTokens } = await import("../drizzle/schema");
    const { eq, and, gt, not } = await import("drizzle-orm");
    ...
    console.error("[DB] Error getting password reset token:", error);
    return null;
  }
}
...
export async function markPasswordResetTokenAsUsed(tokenId: number) {
  ...
  try {
    ...
    console.error("[DB] Error marking token as used:", error);
    throw error;
  }
}
